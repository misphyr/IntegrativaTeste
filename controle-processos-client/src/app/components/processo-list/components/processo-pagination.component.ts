...
@Component({
  selector: 'app-processo-pagination',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="bg-gray-50 border-t border-gray-200 px-6 py-4 flex items-center justify-between">
      <div class="text-sm text-gray-600">
        Mostrando {{ (paginaAtual - 1) * itensPorPagina + 1 }} a {{ Math.min(paginaAtual * itensPorPagina, total) }} de {{ total }} processos
      </div>
      <div class="flex gap-2">
        <button (click)="onPageChange(paginaAtual - 1)" 
          [disabled]="paginaAtual === 1"
          class="px-3 py-2 rounded border border-gray-300 text-gray-700 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">
          ← Anterior
        </button>
...
export class ProcessoPaginationComponent {
  @Input() paginaAtual = 1;
